      ...
      <nav className="my-[8px] md:my-4 lg:m-4 z-50 bg-[var(--portfolio-dark-card)] rounded-3xl h-auto md:h-16">
        <div className="flex items-center justify-between h-full px-8 py-[30px] md:px-8 md:py-0">
          ...
          <div className="hidden md:flex space-x-8 fade-in-up">
            {navLinks.map((link) => (
              <button
                ...
                className="text-[var(--portfolio-bg)] hover:text-[#6c757d] font-medium transition-colors"
              >
                {link.label}
              </button>
            ))}
          ...
          <button
            ...
            className="md:hidden fade-in-up text-[var(--portfolio-bg)] relative z-[60]"
          >
      ...
      <div className={`mobile-menu ${isMenuOpen ? "open" : ""}`}>
        ...
        {navLinks.map((link) => (
          <button
            key={link.href}
            onClick={() => scrollToSection(link.href)}
            className="text-[var(--portfolio-bg)] text-2xl font-medium hover:text-[#6c757d] transition-colors"
          >
            {link.label}
          </button>
        ))}
